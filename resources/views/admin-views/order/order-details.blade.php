        ...
        <div class="col-lg-8">
            ...
            <div class="card h-100">
                ...
                <div class="card-body">
                    <div class="d-flex flex-wrap gap-10 justify-content-between mb-4">
                        ...
                        <div class="text-sm-right">
                            <div class="d-flex flex-wrap gap-10 justify-content-end">
                                ...
                                <div>
                                    <button class="btn btn--primary px-1" data-toggle="modal" data-target="#order_verification_modal"><i
                                ...
                                <div class="">
                                    <button class="btn btn--primary px-1" data-toggle="modal" data-target="#locationModal"><i
                                            class="tio-map"></i> {{translate('show_locations_on_map')}}</button>
                                </div>
                                @endif
                                {{-- download order invoice --}}
                                {{-- <a class="btn btn--primary px-1" target="_blank"
                                        href={{route('admin.orders.generate-invoice',[$order['id']])}}>
                                <img src="{{ asset('assets/back-end/img/icons/uil_invoice.svg') }}" alt="" class="mr-1">
                                {{translate('print_Invoice')}}
                                </a> --}}

                                {{-- print order invoice in new small window --}}
                                <span onclick="orderInvoicePrintWindow(`{{url('/')}}`,'/admin/orders/print-order-invoice',{{$order['id']}},{{$order['id']}},'800','600')" class="btn btn--primary px-1">
                                    <img src="{{ asset('assets/back-end/img/icons/uil_invoice.svg') }}" alt="" class="mr-1">
                                    {{translate('print_Invoice')}}
                                </span>
                                ...
                                <span onclick="sendToSteadFastSingleProduct(`{{$order['id']}}`)" class="btn btn-success px-1">
                                    Send to SteadFast
                                </span>
                                @endif
                                        ...
                                        <div class="media align-items-center gap-10">
                                            ...
                                            <div>
                                                <h6 class="title-color">
                                                    <a href="{{route('admin.product.view',[$detail->product_all_status['id']])}}" class="title-color hover-c1">
                    ...
                    <div class="row justify-content-md-end mb-3">
                        <div class="col-md-9 col-lg-8">
                            <dl class="row gy-1 text-sm-right">
                                ...
                                @if(($total+$shipping-$coupon_discount -$delivery_fee_discount)==$order->order_amount)
                                ...
                                @else
                                @if($order->special_discount)
                                <dt class="col-5">{{translate('special_discount')}}</dt>
                                <dd class="col-6 title-color">
                                    - <strong>{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($order->special_discount))}}</strong>
                                </dd>
                                @endif

                                @if($order->additional_price)
                                <dt class="col-5">{{translate('additional_price')}} <small>({{ $order->additional_price_note }})</small></dt>
                                <dd class="col-6 title-color">
                                    + <strong>{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($order->additional_price))}}</strong>
                                </dd>
                                @endif
                    ...
                    <!-- Check delivery status by using courier api -->
                    @if($order->order_status =='out_for_delivery' || $order->order_status =='delivered')
                    <!-- Delivery status checking btn -->
                    <div class="my-5">
                        <span class="btn btn--primary" onclick="checkDeliveryStatus(`{{$order->third_party_delivery_tracking_id}}`)">Check Delivery Status</span>
                    </div>
                    <div>
                        <div class="mb-2">
                            <h3 class="text-center"><span id="delivery_status_now"></span></h3>
                        </div>

                        <iframe class="d-none border-0" id="delivery_status_result" src="" style="width: 100%; height: 600px;"></iframe>
                    </div>
                ...
                <div class="card-body">
                    ...
                    <div class="d-flex flex-column gap-2">
                        ...
                        <div>
                            ...
                            @if(isset($shipping_address) && property_exists($shipping_address, 'thana') && $shipping_address->thana)
                            <strong>{{$shipping_address->thana}}</strong>
                            @endif
                        </div>
                        <div>
                        ...
                        <div style="padding-bottom: 4px;">
                            ...
                            @if(isset($shipping_address) && property_exists($shipping_address, 'order_comment') && $shipping_address->order_comment)
                            <strong>{{$shipping_address->order_comment}}</strong>
                            @endif
                        </div>


                        <div style="border-top:2px solid gray; padding-top: 4px;">
                            <span>{{translate('hidden_note')}} :</span>
                            @if(isset($shipping_address) && property_exists($shipping_address, 'hidden_note') && $shipping_address->hidden_note)
                            <strong>{{$shipping_address->hidden_note}}</strong>
                            @endif
                        </div>
                ...
                <div class="card-body">
                    <div class="d-flex gap-2 align-items-center justify-content-between mb-4">
                        <h4 class="d-flex gap-2">
                            ...
                            {{translate('edit_balance')}}
                        </h4>

                        @if($order->order_status == "pending")
                        <button class="btn btn-outline-primary btn-sm square-btn" title="Edit" onclick="calculatePayableBalance()" data-toggle="modal" data-target="#payableBalanceUpdateModal">
                            <i class="tio-edit"></i>
                        </button>
                        @else
                        <span></span>
                    ...
                    <div class="d-flex flex-column gap-2">
                        <div>
                            <span>{{translate('previous_balance')}}: </span>
                            <strong>{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($total+$shipping-$coupon_discount -$delivery_fee_discount))}}</strong>
                        </div>
                        <div class="text-primary">
                            <span>{{translate('payable_balance')}}: </span>
                            <strong>{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($order->order_amount))}}</strong>
                        </div>

                        @if($order->special_discount)
                        <div class="text-danger">
                            <span>{{translate('special_discount')}}: </span>
                            <strong>{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($order->special_discount))}}</strong>
                        </div>
                        <div>
                            <span>{{translate('special_discount_for')}}: </span>
                            <strong>{{ $order->special_discount_for }}</strong>
                        </div>
                        @endif

                        @if($order->additional_price)
                        <div class="text-success">
                            <span>{{translate('additional_price')}}: </span>
                            <strong>{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($order->additional_price))}}</strong>
                        </div>
                        <div>
                            <span>{{translate('additional_price_for')}}: </span>
                            <strong>{{ $order->additional_price_for }}</strong>
                        </div>
                        <div>
                            <span>{{translate('additional_price_note')}} (Customer): </span>
                            <strong>{{ $order->additional_price_note }}</strong>
                        </div>
                        @endif
...
<!-- check customer reputation Modal -->
<div class="modal fade" id="checkCustomerReputationModal" tabindex="-1" aria-labelledby="checkCustomerReputationModal" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header pb-4">
                <h3 class="mb-0 text-center w-100">Customer Reputation</h3>
                <button type="button" class="btn-close border-0" data-dismiss="modal" aria-label="Close"><i class="tio-clear"></i></button>
            </div>
            <div class="modal-body px-4 px-sm-5 pt-0">
                <div class="text-center py-5" id="loading_img">
                    <img class="my-5" width="100" height="100" src="{{asset('assets/back-end/img/loader.gif')}}" alt="Loading">
                </div>

                <div class="d-none" id="reputation_all_result">
                    <div class="row p-2 p-md-5">
                        <div class="col-md-6 my-2">
                            ...
                            <div class="my-2">
                                ...
                                <span id="place_total_order"></span>
                            </div>
                            <div class="my-2 text-success">
                                <span class="font-weight-bold">Completed Orders : </span>
                                <span id="order_completed_percentage"></span>%
                            </div>
                            <div class="my-2 text-danger">
                                <span class="font-weight-bold">Returned Orders : </span>
                                <span id="order_returned_percentage"></span>%
                            </div>
                        </div>

                        <div class="col-md-6 my-2">
                            <div style="width: 200px; height:150px; margin:0 auto;">
                                <canvas id="parcelChart" width="100" height="100"></canvas>
                            </div>
                        </div>
                    </div>
                    <div>
                        <table class="table table-bordered text-center">
                            <thead class="thead-light">
                                <tr>
                                    <th>Courier</th>
                                    <th>Total Orders</th>
                                    <th class="text-success">Success Orders</th>
                                    <th class="text-danger">Returned Orders</th>
                                </tr>
                            </thead>
                            <tbody id="customerReputationResult">
            ...
            <div class="modal-body px-4 px-sm-5 pt-0">
                <form action="{{route('admin.orders.address-update')}}" method="post">
                    ...
                    <div class="d-flex flex-column align-items-center gap-2">
                        ...
                        <div class="row gx-2">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="city" id="city" value="{{$shipping_address ? $shipping_address->city : ''}}" class="form-control" placeholder="{{ translate('ex') }}:{{translate('district')}}" required>
                                </div>
                                {{-- <div class="form-group">
                                        <label for="city">{{ translate('district')}}<span class="text-danger"> *</span></label>
                                <select type="text" class="form-control select-district" name="city" id="city" required>
                                    <option value="">{{translate('your_district')}}</option>
                                    <option value="Rajshahi" @if($shipping_address->city=='Rajshahi') selected @endif>Rajshahi</option>
                                    <option value="Nowgan" @if($shipping_address->city=='Nowgan') selected @endif>Nowgan</option>
                                </select>
                            </div> --}}
                        </div>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="thana" class="title-color">{{translate('thana')}}</label>
                                <input type="text" name="thana" id="0" value="@if(isset($shipping_address) && property_exists($shipping_address, 'thana') && $shipping_address->thana){{$shipping_address->thana }}@endif" class="form-control" placeholder="{{ translate('ex') }}:{{translate('your_thana')}}" required>
                            </div>
                            {{-- <div class="form-group">
                                        <label for="thana">{{ translate('thana')}}<span class="text-danger"> *</span></label>
                            <select type="text" class="form-control select-thana" name="thana" id="thana" required>
                                <option value="">{{translate('your_thana')}}</option>
                                <option value="Boalia" @if(isset($shipping_address) && property_exists($shipping_address, 'thana' ) && $shipping_address->thana) @if($shipping_address->thana == 'Boalia') selected @endif @endif>Boalia</option>
                                <option value="Puthia" @if(isset($shipping_address) && property_exists($shipping_address, 'thana' ) && $shipping_address->thana) @if($shipping_address->thana == 'Rajshahi') selected @endif @endif>Puthia</option>
                                <option value="mohanpur" @if(isset($shipping_address) && property_exists($shipping_address, 'thana' ) && $shipping_address->thana) @if($shipping_address->thana == 'mohonpur') selected @endif @endif>mohanpur</option>
                            </select>
                        </div> --}}
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            @else
                            <input type="text" class="form-control" value="{{$shipping_address ? $shipping_address->zip  : ''}}" id="zip" name="zip" placeholder="{{ translate('ex') }}: 1216" {{$shipping_address?'':''}}>
                            @endif
                        </div>
                    </div>
                    <div class="col-9">
                        <div class="form-group">
                            <label for="address" class="title-color">{{translate('village_para_area')}}</label>
                            <input name="address" id="address" name="address" rows="3" class="form-control" placeholder="{{ translate('ex') }} : {{translate('your_village_para_area')}}" value="{{$shipping_address ? $shipping_address->address : ''}}">
                        </div>
                    </div>

                    <div class="col-12">
                        <div class="form-group">
                            <label for="order_comment" class="title-color">{{translate('order_comment')}}</label>
                            <textarea name="order_comment" id="order_comment" name="order_comment" rows="3" class="form-control" placeholder="{{ translate('ex') }} : {{translate('your_comment_about_order')}}">@if(isset($shipping_address) && property_exists($shipping_address, 'order_comment') && $shipping_address->order_comment){{$shipping_address ? $shipping_address->order_comment : ''}}@endif</textarea>
                        </div>
                    </div>
                    ...
                    <div class="col-12">
                        <div class="form-group">
                            <label for="hidden_note" class="title-color">{{translate('hidden_note')}}</label>
                            <textarea name="hidden_note" id="hidden_note" name="hidden_note" rows="3" class="form-control" placeholder="{{ translate('ex') }} : {{translate('hidden_note_about_order')}}">@if(isset($shipping_address) && property_exists($shipping_address, 'hidden_note') && $shipping_address->hidden_note){{$shipping_address ? $shipping_address->hidden_note : ''}}@endif</textarea>
                        </div>
                    </div>
...
<!-- End Modal -->
<!-- payable balance Modal -->
<div class="modal fade" id="payableBalanceUpdateModal" tabindex="-1" aria-labelledby="payableBalanceUpdateModal" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-md">
        <div class="modal-content">
            <div class="modal-header pb-4">
                <h2 class="mb-0 text-center w-100">{{translate('payable_balance')}}</h2>
                <button type="button" class="btn-close border-0" data-dismiss="modal" aria-label="Close"><i class="tio-clear"></i></button>
            </div>
            <div class="modal-body px-4 px-sm-5 pt-0">
                <div class="p-2 p-md-5">
                    <form action="{{route('admin.orders.payable-balance-update')}}" method="post">
                        @csrf
                        <div class="d-flex flex-column align-items-center gap-2">
                            <input name="order_id" value="{{$order->id}}" hidden>
                            <div class="row gx-2">
                                ...
                                <div class="col-12" style="min-height: 260px;">
                                    <div class="mb-2">
                                        <h3 class="font-weight-bold">{{translate('previous_balance')}}: {{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($total+$shipping-$coupon_discount -$delivery_fee_discount))}}</h3>
                                        <input type="text" name="hidden_order_amount" id="hidden_order_amount" value="{{$total+$shipping-$coupon_discount -$delivery_fee_discount}}" hidden>
                                    </div>
                                    <div class="form-group">
                                        <label for="order_amount" class="title-color">{{translate('payable_balance')}}:</label>
                                        <input type="text" name="order_amount" id="order_amount" class="form-control" value="{{$order->order_amount}}" required onkeyup="calculatePayableBalance()">
                                        <span class="mt-1">{{translate('payable_balance_hints')}}</span>
                                    </div>
                                    <div id="specialDiscountSection" class="d-none">
                                        <div class="form-group">
                                            <h4 class="font-weight-bold text-danger">{{translate('special_discount')}}: <span id="special_discount_result">{{$order->special_discount}}</span></h4>
                                            <input type="text" name="special_discount" id="special_discount" value="{{$order->special_discount}}" hidden>
                                            <label for="special_discount_for" class="title-color">{{translate('special_discount_for')}} (only Admin):</label>
                                            <input class="form-control" type="text" name="special_discount_for" id="special_discount_for" value="{{$order->special_discount_for}}">
                                        </div>
                                    </div>

                                    <div id="additionalPriceSection" class="d-none">
                                        <div class="form-group">
                                            <h4 class="font-weight-bold text-success">{{translate('additional_price')}}: <span id="additional_price_result">{{$order->additional_price}}</span></h4>
                                            <input type="text" name="additional_price" id="additional_price" value="{{$order->additional_price}}" hidden>
                                            <label for="additional_price_for" class="title-color">{{translate('additional_price_for')}} (only Admin):</label>
                                            <input class="form-control" type="text" name="additional_price_for" id="additional_price_for" value="{{$order->additional_price_for}}">
                                        </div>
                                        <div class="form-group">
                                            <label for="additional_price_note" class="title-color">{{translate('additional_price_note')}} (Customer & Invoice):</label>
                                            <input class="form-control" type="text" name="additional_price_note" id="additional_price_note" value="{{$order->additional_price_note}}" required>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-12">
...
<script type="text/javascript">
    // print order invoice in new window
    function orderInvoicePrintWindow(appUrl, route, id, title, w, h) {
        var url = appUrl + route + '/' + id;
        var printWindow = window.open(url, title, 'width=' + w + ',height=' + h);
...
<script>
    ...
    function calculatePayableBalance() {
        var mainBalance = parseInt(document.getElementById('hidden_order_amount').value);
        var payableBalance = parseInt(document.getElementById('order_amount').value);
        if ((mainBalance - payableBalance) > 0) {
            var specialDiscount = mainBalance - payableBalance;
            $('#special_discount_result').text(specialDiscount);
            $('#special_discount').val(specialDiscount);

            $('#specialDiscountSection').removeClass('d-none');
            $('#specialDiscountSection').addClass('d-block');
            $('#additionalPriceSection').removeClass('d-block');
            $('#additionalPriceSection').addClass('d-none');

            $('#additional_price_result').text('0');
            $('#additional_price').val(0);
            $('#additional_price_for').val('');
            $('#additional_price_note').val('');
            $('#additional_price_note').removeAttr("required");

        } else if ((mainBalance - payableBalance) < 0) {
            var additionalPrice = payableBalance - mainBalance;
            $('#additional_price_result').text(additionalPrice);
            $('#additional_price').val(additionalPrice);

            $('#additionalPriceSection').removeClass('d-none');
            $('#additionalPriceSection').addClass('d-block');
            $('#specialDiscountSection').removeClass('d-block');
            $('#specialDiscountSection').addClass('d-none');

            $('#special_discount_result').text('0');
            $('#special_discount').val(0)
            $('#special_discount_for').val('')
            $('#additional_price_note').attr("required", "required");
        } else {
            $('#special_discount_result').text('0');
            $('#special_discount').val(0);
            $('#additional_price_result').text('0');
...
{{-- check customer reputation by api --}}
<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
<script type="text/javascript">
    function checkCustomerReputation(phone, name) {
        $('#customer_name').text(name);
        $('#customer_phone').text(phone);
        $.ajax({
            url: "{{ route('admin.orders.check-customer-reputation', '') }}/" + phone,
            type: 'GET',
            headers: {
            ...
            success: function(response) {
                ...
                let $tbody = $('#customerReputationResult');
                $tbody.empty();

                // Initialize totals
                let totalParcelsSum = 0;
                let deliveredParcelsSum = 0;
                let returnedParcelsSum = 0;

                // Process each courier service
                $.each(summaries, function(service, data) {
                    let totalParcels = data['Total Parcels'] || data['Total Delivery'] || 0;
                    let deliveredParcels = data['Delivered Parcels'] || data['Successful Delivery'] || 0;
                    let returnedParcels = data['Canceled Parcels'] || data['Canceled Delivery'] || 0;

                    // Add to the sums
                    totalParcelsSum += parseInt(totalParcels);
                    deliveredParcelsSum += parseInt(deliveredParcels);
                    returnedParcelsSum += parseInt(returnedParcels);
                    ...
                    $tbody.append(`
                        <tr>
                            ...
                            <td>${totalParcels}</td>
                            <td class="text-success">${deliveredParcels}</td>
                            <td class="text-danger">${returnedParcels}</td>
                        </tr>
                    `);
                });

                // Calculate percentages
                let totalSum = deliveredParcelsSum + returnedParcelsSum;
                let deliveredPercentage = totalSum > 0 ? (deliveredParcelsSum / totalSum * 100).toFixed(2) : 0;
                let returnedPercentage = totalSum > 0 ? (returnedParcelsSum / totalSum * 100).toFixed(2) : 0;

                // view percentage resutl below mobile
                $('#place_total_order').text(totalParcelsSum);
                $('#order_completed_percentage').text(deliveredPercentage);
                $('#order_returned_percentage').text(returnedPercentage);
                // Append the totals row
                $tbody.append(`
                    <tr class="font-weight-bold">
                        <td>Total</td>
                        <td>${totalParcelsSum}</td>
                        <td class="text-success">${deliveredParcelsSum} (${deliveredPercentage}%)</td>
                        <td class="text-danger">${returnedParcelsSum} (${returnedPercentage}%)</td>
                    </tr>
                `);
                // Create the chart
                var ctx = document.getElementById('parcelChart').getContext('2d');
                var parcelChart = new Chart(ctx, {
                    type: 'pie',
                    data: {
                        labels: ['Completed', 'Returned'],
                        datasets: [{
                            data: [deliveredParcelsSum, returnedParcelsSum],
                            backgroundColor: ['#00C9A7', '#EE5780'], // green for delivered, red for Returned
                        }]
                    },
                    options: {
                        responsive: true,
                ...
                $('#reputation_all_result').removeClass('d-none');
                $('#reputation_all_result').addClass('d-block');
            },
...
<script type="text/javascript">
    // sent to stead fast courier button
    function sendToSteadFastSingleProduct(id) {
        $.ajax({
            url: "{{ route('admin.orders.send-single-product', '') }}/" + id,
            type: 'GET',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
            },
            ...
            error: function(xhr, status, error) {
                ...
                toastr.error('{{ translate("sending_error") }}');
            }
        });
    }
</script>
...
<script type="text/javascript">
    function checkDeliveryStatus(trackingID) {
        $.ajax({
            url: "{{ route('admin.orders.check-delivery-status', '') }}/" + trackingID,
            type: 'GET',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
            },
            success: function(data) {
                ...
                $('#delivery_status_now').html('<span class="spinner-grow spinner-grow-sm mb-1" role="status"><span class="sr-only"></span></span> Status : ' + data.delivery_status + '<hr>');
            },
            error: function(error) {
                console.error('Error:', error);
            }
        ...
        $('#delivery_status_result').attr('src', 'https://steadfast.com.bd/t/' + trackingID);
        $('#delivery_status_result').removeClass('d-none');
        $('#delivery_status_result').addClass('d-block');
    }
</script>
...
<script>
    ...
    function show_location_map() {
        ...
        google.maps.event.addListener(marker, 'click', (function(marker) {
            return function() {
                infowindow.setContent("<div class='float-left'><img class='__inline-5' src='{{asset('storage/profile/')}}{{$order->customer->image??"
