                ...
                <div class="card-body">
                    ...
                    <div class="px-3 py-4 light-bg">
                        <div class="row g-2 align-items-center flex-grow-1">
                            ...
                            {{-- sent to Courier button --}}
                            @if($status =='processing')
                                <div>
                                    <button id="sendToSteadFastBtn" class="btn btn--primary input-group-text" disabled>Send to SteadFast</button>
                                </div>
                            @endif
                            {{-- label generate button --}}
                            @if($status =='out_for_delivery')
                                <div>
                                    <button id="labelGenerateBtn" class="btn btn--primary input-group-text" disabled>Generate Label</button>
                                </div>
                            @endif
                    ...
                    <div class="table-responsive datatable-custom">
                        <table class="table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table w-100"
                            ...
                            @foreach($orders as $key=>$order)
                                ...
                                {{-- checkbox to send product to courier --}}
                                    @if($status =='processing')
                                    <td class="pr-0 mx-0">
                                        <input type="checkbox" name="orderId[]" style="cursor: pointer;" id="{{$order['id']}}" value="{{$order['id']}}">
                                    </td>
                                    @endif
                                {{-- checkbox to generate label --}}
                                    @if($status =='out_for_delivery')
                                    <td class="pr-0 mx-0">
                                        <input type="checkbox" name="orderId[]" style="cursor: pointer;" id="{{$order['id']}}" value="{{$order['id']}}">
                                    </td>
...
@push('script_2')
 ...
 // send to stead fast bulk product
     document.addEventListener('DOMContentLoaded', () => {
        const checkboxes = document.querySelectorAll('input[name="orderId[]"]');
        const sendToSteadFastBtn = document.getElementById('sendToSteadFastBtn');
        ...
        checkboxes.forEach(checkbox => {
            checkbox.addEventListener('change', () => {
                ...
                sendToSteadFastBtn.disabled = !anyChecked;
            });
        });

        // Handle button click
        sendToSteadFastBtn.addEventListener('click', () => {
            const checkedValues = Array.from(checkboxes)
                .filter(chk => chk.checked)
                .map(chk => chk.value);

            $.ajax({
                url: "{{ route('admin.orders.send-bulk-products') }}",
                type: 'POST',
                contentType: 'application/json',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
                },
                data: JSON.stringify({ orderIds: checkedValues }),
 ...
 // send to stead fast bulk product
     document.addEventListener('DOMContentLoaded', () => {
        const checkboxes = document.querySelectorAll('input[name="orderId[]"]');
        const labelGenerateBtn = document.getElementById('labelGenerateBtn');

        // Toggle button enabled/disabled based on checkbox state
        checkboxes.forEach(checkbox => {
            checkbox.addEventListener('change', () => {
                const anyChecked = Array.from(checkboxes).some(chk => chk.checked);
                labelGenerateBtn.disabled = !anyChecked;
            });
        });

        // Handle button click
        labelGenerateBtn.addEventListener('click', () => {
            const checkedValues = Array.from(checkboxes)
                .filter(chk => chk.checked)
                .map(chk => chk.value);

            $.ajax({
                url: "{{ route('admin.orders.generate-label') }}",
                type: 'POST',
                contentType: 'application/json',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
