...
<head>
    ...
    <title>{{ $order->id }} {{ ($order->order_type == 'POS' ? '(POS Order)' : '' ) }}</title>
    ...
    <table class="bs-0 mb-30 px-10">
        <tr>
            ...
            <th class="content-position-y text-right">
                ...
                <h4 class="fz-14 pr-1">Date : {{date('d-m-Y',strtotime($order['created_at']))}}</h4>
            </th>
        </tr>
    </table>
</div>
                            ...
                            <td>
                                @if($order->shipping_address_data)
                                    ...
                                    <div class="h4 montserrat-normal-600">
                                        ...
                                        <p style=" margin-top: 6px; margin-bottom:0px;">@if(isset($shipping_address) && property_exists($shipping_address, 'thana') && $shipping_address->thana){{$shipping_address->thana}}@endif</p>
                            ...
                            <td class="text-right">
                                @if($order->billing_address_data)
                                    ...
                                    <div class="h4 montserrat-normal-600">
                                        ...
                                        <p style=" margin-top: 6px; margin-bottom:0px;">@if(isset($billingAddress) && property_exists($billingAddress, 'thana') && $billingAddress->thana){{$billingAddress ? $billingAddress->thana : ''}}@endif</p>
                                ...
                                @elseif($order->billingAddress)
                                    ...
                                    <div class="h4 montserrat-normal-600">
                                        ...
                                        <p style=" margin-top: 6px; margin-bottom:0px;">@if(isset($order->billingAddress) && property_exists($order->billingAddress, 'thana') && $order->billingAddress['thana']){{$order->billingAddress ? $order->billingAddress['thana'] : ''}}@endif</p>
            ...
            <th>
                <table class="calc-table">
                    <tbody>
                        ...
                        <tr>
                            <td class="p-1 text-left">Special Discount For You</td>
                            <td class="p-1">
                                - {{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($order->special_discount))}} </td>
                        </tr>
                        @endif

                        @if($order->additional_price)
                        <tr>
                            <td class="p-1 text-left">Additional price <small>({{ $order->additional_price_note }})</small></td>
                            <td class="p-1">
                                - {{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($order->additional_price))}} </td>
                        </tr>
                        @endif
                        <tr>
