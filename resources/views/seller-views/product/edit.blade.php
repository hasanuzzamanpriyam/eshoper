...
<div class="content container-fluid">
    ...
    <form class="product-form text-start" action="{{route('seller.product.update', $product->id)}}" method="post"
        ...
        <div class="card">
            <div class="px-4 pt-3">
                @php($pnc_language = \App\Model\BusinessSetting::where('type', 'pnc_language')->first())
                @php($pnc_language_value = $pnc_language ? $pnc_language->value : null)
                @php($languages = $pnc_language_value ? json_decode($pnc_language_value, true) : ['en'])
                @php($default_lang = $languages[0] ?? 'en')
                                ...
                                <div class="{{$lang != 'en' ? 'd-none' : ''}} lang_form" id="{{$lang}}-form">
                                    ...
                                    @if($lang == $default_lang)
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="slug" id="slug" class="form-control" placeholder="Ex: apple-iphone-12" value="{{ $product->slug }}" required>
                    ...
                    <div class="col-md-6 col-lg-4 col-xl-3">
                        <div class="form-group">
                            ...
                            <input type="number" min="0" step="0.01" placeholder="{{translate('purchese_price') }}"
                                name="purchase_price" class="form-control" value={{ \App\CPU\Convert::default($product->purchase_price) }} required>
                        </div>
                    </div>
                    <div class="col-md-6 col-lg-4 col-xl-3">
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <div class="d-flex gap-2">
                                <label class="title-color" for="discount">
                                    ...
                                    <span
                                        class="discount_amount_symbol">({{ $product->discount_type == 'flat' ? \App\CPU\BackEndHelper::currency_symbol() : '%' }})</span>
                                </label>
                            ...
                            <input type="number" min="0" value={{ $product->discount_type == 'flat' ? \App\CPU\Convert::default($product->discount) : $product->discount}} step="0.01" placeholder="{{translate('ex: 5') }}" name="discount"
                            ...
                            <div class="mt-2">
                                <label class="switcher">
                                    <input type="checkbox" class="switcher_input" name="is_delivery_free"
                                        id="is_delivery_free" {{ $product->is_delivery_free == 1 ? 'checked' : '' }}>
                            ...
                            <div class="mt-2">
                                <label class="switcher">
                                    <input type="checkbox" class="switcher_input" name="is_cash_on_delivery"
                                        id="is_cash_on_delivery" {{ $product->is_cash_on_delivery == 1 ? 'checked' : '' }}>
                                    <span class="switcher_control"></span>
                                </label>
                            </div>
                        </div>
