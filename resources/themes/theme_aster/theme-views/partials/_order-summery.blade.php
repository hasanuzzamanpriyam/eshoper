    ...
    <div class="card text-dark sticky-top-80">
        <div class="card-body px-sm-4 d-flex flex-column gap-2">
            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mt-4">
                ...
                <a {{ Request::is('shop-cart')?"onclick=checkout()":"" }} {{ Request::is('checkout-details')?"onclick=proceed_to_next()":"" }} {{ in_array($current_url, ["updateQuantity", "remove"]) ? "onclick=checkout()":"" }} class="btn btn-primary {{ $cooldownRemaining > 0 ? 'disabled' : '' }}">{{ translate('Proceed_to_Next') }}</a>
        ...
        function updateTimer() {
            ...
            if (window.cooldownSeconds > 0) {
                ...
                if (currentRoute === 'checkout-payment') {
                    ...
                    document.querySelectorAll('.btn-primary, .proceed_to_next_button').forEach(btn => {
