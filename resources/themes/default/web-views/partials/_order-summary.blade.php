    ...
    <div class="__cart-total __cart-total_sticky">
        ...
        <div class="mt-4">
            @if($web_config['guest_checkout_status'] || auth('customer')->check())
                <a onclick="checkout()" class="btn btn--primary btn-block proceed_to_next_button {{$cart->count() <= 0 ? 'disabled' : ''}}" >{{Route::currentRouteName() == 'checkout-payment' ? translate('proceed_to_payment') : translate('proceed_to_next')}}</a>
            @else
...
<div class="bottom-sticky3 bg-white p-3 shadow-sm w-100 d-lg-none">
    ...
    @if($web_config['guest_checkout_status'] || auth('customer')->check())
        <a onclick="checkout()" class="btn btn--primary btn-block proceed_to_next_button text-capitalize {{$cart->count() <= 0 ? 'disabled' : ''}}">{{Route::currentRouteName() == 'checkout-payment' ? translate('proceed_to_payment') : translate('proceed_to_next')}}</a>
    @else
...
@push('script')
    <script>
        var cartData = {
            ecommerce: {
                currency: 'BDT',
                value: {{$sub_total+$total_tax+$total_shipping_cost-$coupon_dis-$total_discount_on_product-$order_wise_shipping_discount}},
                items: [
