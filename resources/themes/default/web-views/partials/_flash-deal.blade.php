...
<style>
    ...
    @media (max-width: 767px) {
        ...
        .view-all-btn-custom {
            ...
            <div class="d-flex flex-column flex-md-row align-items-center justify-content-between gap-3">
                ...
                <div class="text-start">
                    <div class="flash-deal-text" style="color: {{$web_config['primary_color']}};">
                        <div>
                            <span class="fw-bold fs-3">{{$flash_deal->title}}</span>
                        </div>
                ...
                <div class="text-center text-md-end">
                    @if (count($flash_deal->products)>0)
                        <a class="text-capitalize view-all-text d-flex align-items-center justify-content-center justify-content-md-end gap-1 view-all-btn-custom"
                           href="{{route('flash-deals',[$flash_deal?$flash_deal['id']:0])}}">
                            {{ translate('view_all')}}
            ...
            <!-- Banner Section -->
            @if(file_exists(public_path('storage/deal/'.$flash_deal->banner)))
                @php($deal_banner = asset('storage/deal/'. $flash_deal->banner))
            ...
            <div class="row g-3 mx-max-md-0">
                @php($count = 0)
                @foreach($flash_deal->products as $key=>$deal)
