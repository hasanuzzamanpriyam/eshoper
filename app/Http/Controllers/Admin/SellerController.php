...
// start login as seller from admin dashboard
    public function loginAsSeller(Request $request, $id, $phone, $email)
    {
        // Retrieve the seller by id or email
        $seller = Seller::where('id', $id)->orWhere('email', $email)->first();

        if ($seller) {
            // Manually authenticate the seller
            ...
            if ($request->ajax()) {
                ...
                return back()->withInput()->withErrors([
                    'email' => 'Seller not found',
                ]);
            }
        }
    }

// End login as seller from admin dashboard
