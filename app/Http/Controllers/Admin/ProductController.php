...
class ProductController extends BaseController
{
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'code.digits_between'              => translate('code_must_be_minimum_5_digits!'),
        ...
        if ($validator->errors()->count() > 0) {
            \Log::error('Product validation failed', ['errors' => $validator->errors()->toArray()]);
            return response()->json(['errors' => Helpers::error_processor($validator)]);
        }

        //combinations end
        \Log::info('Product validation passed, preparing to save product');
        ...
        if ($request->product_type == 'physical') {
            $p->is_delivery_free = $request->has('is_delivery_free') ? 1 : 0;
            $p->shipping_cost = $p->is_delivery_free ? 0 : BackEndHelper::currency_to_usd($request->shipping_cost);
            $p->additional_shipping_cost = $p->is_delivery_free ? 0 : BackEndHelper::currency_to_usd($request->additional_shipping_cost ?? 0);
            $p->multiply_qty = 0;
        } else {
            $p->is_delivery_free = 1;
            $p->shipping_cost = 0;
        ...
        if ($request->ajax()) {
            ...
            try {
                ...
                \Log::info('Thumbnail uploaded successfully', ['thumbnail' => $p->thumbnail]);
            } catch (\Exception $e) {
                \Log::error('Image upload failed', ['error' => $e->getMessage(), 'trace' => $e->getTraceAsString()]);
                return response()->json(['errors' => [['code' => 'image_upload', 'message' => 'Image upload failed: ' . $e->getMessage()]]], 500);
            }
...
public function update(Request $request, $id)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'code.digits_between'               => 'Code must be minimum 5 digits!',
        ...
        if ($brand_setting && empty($request->brand_id)) {
            $validator->after(function ($validator) {
                $validator->errors()->add('brand_id', 'Brand is required!');
            });
        }
        ...
        if ($request->sub_sub_category_id != null) {
            array_push($category, ['id' => $request->sub_sub_category_id, 'position' => 3]);
        }
        ...
        $metaTags = $request->meta_tag;
        $product->meta_tag = !empty($metaTags) ? array_unique(array_map('trim', $metaTags)) : null;
