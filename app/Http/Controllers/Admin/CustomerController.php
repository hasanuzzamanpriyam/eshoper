...
// login as customer from admin dashboard
    public function loginAsCustomer(Request $request, $id)
    {
        ...
        if ($user) {
            ...
            // Ensure a valid redirect URL
            $redirect_url = session('keep_return_url', route('home'));
            ...
            if ($request->ajax()) {
                ...
                return redirect($redirect_url); // Ensure it's a valid string URL
            }
        } else {
            if ($request->ajax()) {
                return response()->json([
                    'status' => 'error',
                    'message' => translate('login_failed'),
                ]);
            } else {
...
// end login as customer from admin dashboard
