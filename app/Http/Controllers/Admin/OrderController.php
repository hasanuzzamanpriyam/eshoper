...
// generate order invoice and download
...
// Generate and Print order invoice
 public function print_order_invoice($id)
    {
        $company_phone =BusinessSetting::where('type', 'company_phone')->first()->value;
        $company_email =BusinessSetting::where('type', 'company_email')->first()->value;
        $company_name =BusinessSetting::where('type', 'company_name')->first()->value;
        ...
        return view('admin-views.order.print-invoice',
            compact('order', 'seller', 'company_phone', 'company_name', 'company_email', 'company_web_logo'));
    }
    ...
    public function generate_label(Request $request)
    {
        ...
        return response($allProductData);
...
// payable balance update
    public function payable_balance_update(Request $request){
        $validated = $request->validate([
            'order_amount' => 'required|numeric',
            'special_discount' => 'nullable|numeric',
            'special_discount_for' => 'nullable|string|max:255',
            'additional_price' => 'nullable|numeric',
            'additional_price_for' => 'nullable|string|max:255',
            'additional_price_note' => 'nullable|string|max:500',
        ]);
        $payableBalanceUpdate = DB::table('orders')
            ->where('id', $request->order_id)
            ->update([
                'order_amount' => $request->order_amount,
                'special_discount' => $request->special_discount,
                'special_discount_for' => $request->special_discount_for,
                'additional_price' => $request->additional_price,
                'additional_price_for' => $request->additional_price_for,
                'additional_price_note' => $request->additional_price_note,
            ]);

        if($payableBalanceUpdate){
            Toastr::success(translate('successfully_updated'));
        }else{
            Toastr::error(translate('problem_to_updated'));
        }
        return back();
    }
