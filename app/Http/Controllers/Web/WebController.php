...
class WebController extends Controller
{
	...
	public function checkout_details(Request $request)
	{
		...
		// আইপি ট্র্যাকিং এবং কুলডাউন চেক
		// Check cooldown but don't redirect, just pass time to view
		$ipAddress = $request->ip();
		$cooldownRemaining = 0;
		$lastOrder = OrderIpRestriction::where('ip_address', $ipAddress)->orderBy('last_order_time', 'desc')->first();
		if ($lastOrder && Carbon::parse($lastOrder->last_order_time)->addMinutes(5) > Carbon::now()) {
			$cooldownRemaining = Carbon::now()->diffInSeconds(Carbon::parse($lastOrder->last_order_time)->addMinutes(5));
		}
	...
	public function checkout_complete(Request $request)
	{
		...
		// আইপি ট্র্যাকিং এবং কুলডাউন চেক
		$ipAddress = $request->ip();
		if (OrderManager::check_repeat_order_cooldown($cart_group_ids, $ipAddress) === false) {
			Toastr::error(translate('You cannot place another order within 5 minutes!'));
			return redirect()->route('shop-cart');
		}
