...
class CartController extends Controller
{
    ...
    /**
     * Calculate delivery charge based on selected district (AJAX endpoint for real-time updates)
     */
    public function calculate_delivery_charge(Request $request)
    {
        ...
        // Store district in session for persistence
        session(['selected_district' => $district]);
        session(['selected_district_id' => $district_id]);

        // Get product-based shipping cost (products with shipping_cost > 0)
        $cart_shipping_cost = Cart::where(['product_type' => 'physical'])
            ->whereIn('cart_group_id', CartManager::get_cart_group_ids())
            ->sum('shipping_cost');

        // Get district-based delivery charge (for products with shipping_cost = 0)
        $district_delivery_charge = CartManager::calculate_delivery_charge_by_district($district);

        // Calculate total shipping cost
        $total_shipping = $cart_shipping_cost + $district_delivery_charge;

        // Store shipping cost in session
        session(['shipping_cost_calculated' => $total_shipping]);

        // Get cart for calculations
        $cart = CartManager::get_cart();

        // Calculate subtotal with discounts
        $sub_total = CartManager::cart_total_applied_discount($cart);
        ...
        return response()->json([
            'success' => true,
            'shipping_cost' => Helpers::currency_converter($total_shipping),
            'shipping_cost_value' => $total_shipping,
            'district_charge' => Helpers::currency_converter($district_delivery_charge),
