...
class OrderManager
{
    ...
    public static function check_repeat_order_cooldown($group_ids, $ipAddress = null)
    {
        // Check IP restriction table for everyone (strictest check)
        $ipAddress = $ipAddress ?? request()->ip();
        $lastOrder = OrderIpRestriction::where('ip_address', $ipAddress)
            ->where('last_order_time', '>=', Carbon::now()->subMinutes(5))
            ->first();
